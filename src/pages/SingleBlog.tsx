...
import { Calendar, User, Clock, Facebook, Twitter, Linkedin, ArrowLeft } from 'lucide-react';
...
function SingleBlog() {
  ...
  <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
    {/* Back Button */}
    <Link to="/blog" className="inline-flex items-center text-Lg font-semibold hover:text-Mg transition-colors mb-6">
      ...
      Back to Blog
        ...
        {/* Blog Header */}
        <div className="mb-10">
          <h1 className="text-4xl md:text-5xl font-bold text-Mg">{post.title}</h1>
        ...
        <div className="rounded-3xl overflow-hidden shadow-xl mb-10">
          <img src={post.image} alt={post.title} className="w-full h-[450px] object-cover" />
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {post.tags.map((tag, index) => (
            <span key={index} className="bg-violet-100 text-Lg px-3 py-1 rounded-full text-sm">
              #{tag}
            </span>
          ))}
        </div>
        ...
        <div className="border-t border-b border-Lg py-6 flex justify-between items-center mb-10">
          ...
          <div className="flex space-x-4">
            {[Facebook, Twitter, Linkedin].map((Icon, index) => (
              <button key={index} className="text-Dg hover:text-Lg transition-colors">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {post.relatedPosts.map((relatedPost, index) => (
            <div key={index} className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all">
              ...
              <div className="p-6">
                <h3 className="font-semibold text-Dg mb-2">{relatedPost.title}</h3>
                <p className="text-Dg text-sm">{relatedPost.date}</p>
              </div>
            </div>
          ))}
        </div>
